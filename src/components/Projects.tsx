...
const Projects = () => {
    ...
    }, [jluxImages.length, gcurlyImages.length, cobraImages.length, epImages.length])
            ...
            <div className="max-w-[1500px] mx-auto">
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`px-6 py-3 text-xs font-black uppercase tracking-[0.2em] border-2 transition-all ${
                                    ...
                                    ? "bg-white text-black border-white shadow-[-6px_6px_0_0_#22d3ee]" 
                ...
                <div className="space-y-40">
                    <AnimatePresence mode="popLayout">
                            ...
                            <div className="relative w-full h-[60vh] md:h-[70vh] lg:h-[85vh] overflow-hidden border-[3px] border-orange-500/40 shadow-[0_0_40px_rgba(249,115,22,0.08)] bg-black flex items-center justify-center p-4 group/img">
                                ...
                                <div className="relative w-full max-w-5xl h-full flex items-center justify-center gap-4 md:gap-12 lg:gap-20">
                                    ...
                                    >
                                        <AnimatePresence mode="wait">
                                            <motion.img
                                                key={(cobraIndex + 1) % cobraImages.length}
                                                src={cobraImages[(cobraIndex + 1) % cobraImages.length]}
                                    ...
                                    >
                                        <AnimatePresence mode="wait">
                                            <motion.img
                                                key={(cobraIndex + 2) % cobraImages.length}
                                                src={cobraImages[(cobraIndex + 2) % cobraImages.length]}
                                                className="w-full h-full object-cover"
                                                initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                                            />
                                        </AnimatePresence>
                        ...
                        {/* PROYECTO 4: ELECTROPOWER (LANDING) */}
                        {(activeCategory === 'all' || activeCategory === 'landing') && (
                        <motion.div 
                            key="electropower"
                            initial={{ opacity: 0, scale: 0.95 }}
                ...
                >
                    ...
                    >
                        ...
                        <div className="space-y-4 md:space-y-6">
                            {(activeProject === 'jluxuries' || activeProject === 'cobrago') ? (
                                <>
                                    <a
                                        href={activeProject === 'jluxuries' ? "https://github.com/JorgeAndresDev/JLUXURIES-API.git" : "https://github.com/JorgeAndresDev/API-cobrago.git"}
                                        target="_blank"
                                        className={`block w-full py-6 md:py-8 px-8 md:px-10 border-4 border-white/10 bg-[#050505] transition-all group hover:scale-[1.02] ${
                                            activeProject === 'jluxuries' ? 'hover:border-cyan-400' : 'hover:border-orange-400'
                                        }`}
                                    >
                                        <div className="flex justify-between items-center text-white font-black text-lg md:text-2xl uppercase tracking-widest">
                                            <span>Backend / API</span>
                                            <span className={`${
                                                activeProject === 'jluxuries' ? 'text-cyan-500' : 'text-orange-500'
                                            } group-hover:translate-x-4 transition-transform text-3xl md:text-4xl leading-none`}>→</span>
                                        </div>
                                    </a>

                                    <a
                                        href={activeProject === 'jluxuries' ? "https://github.com/JorgeAndresDev/JLUXURIES-Front" : "https://github.com/JorgeAndresDev/cobrago-mobile.git"}
                                        target="_blank"
                                        className={`block w-full py-6 md:py-8 px-8 md:px-10 border-4 border-white/10 bg-[#050505] transition-all group hover:scale-[1.02] ${
                                            activeProject === 'jluxuries' ? 'hover:border-cyan-400' : 'hover:border-orange-400'
                                        }`}
                                    >
                                        <div className="flex justify-between items-center text-white font-black text-lg md:text-2xl uppercase tracking-widest transition-all">
                                ...
                                >
                                    <div className="flex justify-between items-center text-white font-black text-xl md:text-3xl uppercase tracking-widest">
